...
- 在回复模板中，\`{{}}\` 是你需要替换的部分，例如你需要将 \`{{整体意图}}\` 替换为实际的整体意图。
- 在回复模板中，所有的章节、表格等仅列出了一项或两项，但你应该根据实际情况，列出合适的项目数量。
- 下面给出的代码改动通常以 \`diff\` 格式表示，其中的文件路径可能以 \`a/\` 或 \`b/\` 为前缀。在“按文件评审”中，\`{{完整文件路径}}\` **不应该**包含前缀 \`a/\` 或 \`b/\`。
- 在“按文件评审”中，\`{{文件名}}\` 不需要包含其路径。但当存在同名的文件时，应该在 \`{{文件名}}\` 中包含可以使其区分的最短父路径。
